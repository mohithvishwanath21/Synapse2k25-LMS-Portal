import {  Mail, Phone, BadgeIcon as IdCard, Activity, BookOpen , Calendar, PartyPopper} from "lucide-react"
import { useParams } from 'react-router-dom'
import {useAdminLoadUserDetailsQuery} from '@/services/adminApi/adminUserApi'

const StudentDetails = () => {
  const {studentId} = useParams();
  const {data : details} = useAdminLoadUserDetailsQuery(studentId);
  ...
  const detailItems = [
    ...
    { icon: IdCard, label: "Student ID", value: data?._id },
    { icon: Activity, label: "Status", value: data?.isActive ? 'Active' : 'Not Active' },
    { icon: BookOpen, label: "Courses", value: data?.enrolledCourses.join(", ") },
